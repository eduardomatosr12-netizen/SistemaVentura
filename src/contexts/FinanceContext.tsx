...
export const FinanceProvider = ({ children }: { children: ReactNode }) => {
  ...
  const refresh = useCallback(() => {
    setIsLoading(true);
    financeService.fetchTransactions().then(records => {
      setTransactions(records);
      setIsLoading(false);
    }).catch(err => {
      console.error('[Finance] Erro ao atualizar:', err);
      setIsLoading(false);
    });
  }, []);

  return (
    <FinanceContext.Provider value={{ transactions, isLoading, addTransaction, updateTransaction, deleteTransaction, refresh }}>
      {children}
